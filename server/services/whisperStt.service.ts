...
export class WhisperSttService {
  ...
  async transcribeAudio(
    ...
    const apiUrl =
      ...
      process.env.STT_API_URL ||
      this.apiUrl ||
      (apiKey?.startsWith("gsk_") ? "https://api.groq.com/openai/v1/audio/transcriptions" : "https://api.openai.com/v1/audio/transcriptions");
    const model =
      process.env.GROQ_WHISPER_MODEL ||
      process.env.STT_MODEL ||
      this.model ||
      (apiKey?.startsWith("gsk_") ? "whisper-large-v3-turbo" : "whisper-1");
