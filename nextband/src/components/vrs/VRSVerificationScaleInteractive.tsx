    ...
    <div className="space-y-6">
      ...
      {model.passageContext && (
        <div className="border border-slate-200 rounded-2xl p-5 bg-slate-50/70">
          ...
          <div className="space-y-3">
            {model.passageContext.paragraphs.map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className={`p-3.5 rounded-xl transition-all border text-xs leading-relaxed ${
                    isHighlighted
                      ? 'bg-amber-50/90 border-amber-300 text-amber-950 shadow-sm ring-2 ring-amber-400/30'
                      : 'bg-white border-slate-200 text-slate-700'
                  }`}
                >
                  <div className="flex items-center justify-between font-bold text-[11px] mb-1 text-slate-500">
                    <span>{p.label}</span>
                    {isHighlighted && (
                      <span className="text-amber-700 bg-amber-100/80 px-2 py-0.5 rounded-md font-mono text-[10px] flex items-center gap-1 animate-pulse">
                        <ArrowDownRight className="w-3 h-3" /> Tọa độ phát hiện bằng chứng
                      </span>
                    )}
                  </div>
                  <p>
                    {isHighlighted ? (
                      <span>
                        ...
                        <mark className="bg-amber-300/80 text-amber-950 font-bold px-1 py-0.5 rounded border-b-2 border-amber-600">
          ...
          {isLocated && (
            <div className="mt-3 p-2.5 rounded-xl bg-indigo-50/90 border border-indigo-200 flex items-center gap-2 text-xs font-semibold text-indigo-900 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 p-6 rounded-2xl bg-slate-50/50 border border-slate-200">
        ...
        <div className="p-5 rounded-xl bg-white border border-slate-200 shadow-xs flex flex-col justify-between">
          <div>
            ...
            <p className="font-semibold text-sm text-slate-900 mb-4 leading-snug">{model.statement.rawText}</p>
          </div>
          ...
          <div className="flex flex-wrap gap-2 pt-3 border-t border-slate-100">
            {model.statement.deconstructedVariables.map((v, i) => (
              <span
                key={i}
                className={'text-xs px-2.5 py-1 rounded-lg font-medium ' + (v.isTrapWord ? 'bg-rose-50 text-rose-700 font-bold border border-rose-200 flex items-center gap-1' : 'bg-slate-100 text-slate-700')}
              >
                {v.isTrapWord && <AlertTriangle className="w-3 h-3 text-rose-500" />}
                {v.text}
              </span>
            ))}
          </div>
        ...
        <div className="p-5 rounded-xl bg-white border border-slate-200 shadow-xs flex flex-col justify-between">
          <div>
            ...
            <p className="font-semibold text-sm text-slate-900 mb-4 leading-snug">{model.passageEvidence.rawText}</p>
          </div>
          ...
          <div className="flex flex-wrap gap-2 pt-3 border-t border-slate-100">
            {model.passageEvidence.targetVariables.map((v, i) => (
              <span
                key={i}
                className="text-xs px-2.5 py-1 rounded-lg font-bold bg-amber-50 text-amber-800 border border-amber-200"
              >
                🔎 {v.text}
              </span>
            ))}
          ...
          {(['true', 'false', 'not_given'] as const).map((v) => {
            ...
            return (
              <button
                ...
                className={'px-6 py-2.5 rounded-xl font-bold text-xs tracking-wide transition-all shadow-xs ' + (isSelected ? 'bg-indigo-600 text-white scale-105 shadow-md ring-2 ring-indigo-300' : 'bg-white border border-slate-200 text-slate-700 hover:bg-slate-50')}
              >
                {label}
              </button>
            );
